...
resource "aws_instance" "main" {
  ...
  count                = length(data.tfe_outputs.networking.nonsensitive_values.public_subnets) ### maj module 7
  ami                  = nonsensitive(data.aws_ssm_parameter.amzn2_linux.value)
  iam_instance_profile = aws_iam_instance_profile.main.name
  instance_type        = var.instance_type
  ###subnet_id     = var.public_subnets[count.index]
  subnet_id = data.tfe_outputs.networking.nonsensitive_values.public_subnets[count.index] ### maj module 7
  vpc_security_group_ids = [
    aws_security_group.webapp_http_inbound_sg.id,
    ###   aws_security_group.webapp_ssh_inbound_sg.id, ###maj module 9
...
resource "aws_lb" "main" {
  ...
  subnets = data.tfe_outputs.networking.nonsensitive_values.public_subnets ### maj module 7
...
resource "aws_lb_target_group" "main" {
  ...
  protocol    = "HTTP"
  ###vpc_id      = var.vpc_id
  vpc_id = data.tfe_outputs.networking.nonsensitive_values.vpc_id ### maj module 7
}
